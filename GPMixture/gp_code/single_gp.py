...
class singleGP:
    ...
    def __init__(self, startG, endG, goalsData, mode = None):
        ...
        if mode == 'Trautman': #!Using path_regression for now
            ...
            self.gpTrajectoryRegressor = path_regression(self.goalsData.kernelsX[self.startG][self.endG], self.goalsData.kernelsY[self.startG][self.endG],None,None,self.goalsData.areas_coordinates[self.endG],self.goalsData.areas_axis[self.endG],None,'Trautman',timeTransitionData)
        else:
