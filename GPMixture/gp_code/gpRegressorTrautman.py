"""
A class for GP regression for the Trautman code: 
- it uses time as the independent variable
- it does not include the linear prior
"""
import numpy as np
import math
...
class gpRegressorTrautman:
    ...
    def updateObservations(self,observedX,observedY,observedT):
        n                    = len(observedX)
        self.observedX       = np.zeros((n+1,1))
        self.observedY       = np.zeros((n+1,1))
        self.observedT       = np.zeros((n+1,1))
        self.Kx              = np.zeros((n+1,n+1))
        self.Ky              = np.zeros((n+1,n+1))
        # Last really observed point
        lastObservedPoint = [observedX[-1], observedY[-1], observedT[-1]]
        # Generate the set of l values at which to predict x,y
        self.newT, finalL, self.dist = get_prediction_set_T(lastObservedPoint,self.finalAreaCenter,self.unit,self.stepUnit,self.speed)
        # Fill in K (n+1 x n+1)
        for i in range(n):
            self.Kx[i][i] = self.kernelX(observedT[i],observedT[i])
            self.Ky[i][i] = self.kernelY(observedT[i],observedT[i])
            for j in range(i):
                self.Kx[i][j] = self.kernelX(observedT[i],observedT[j])
                self.Kx[j][i] = self.Kx[i][j]
                self.Ky[i][j] = self.kernelY(observedT[i],observedT[j])
                self.Ky[j][i] = self.Ky[i][j]
        # Last row/column
        for i in range(n):
            self.Kx[i][n] = self.kernelX(observedT[i],finalL)
            self.Kx[n][i] = self.Kx[i][n]
            self.Ky[i][n] = self.kernelY(observedT[i],finalL)
        ...
        for i in range(n):
            self.updateObserved(i,observedX[i],observedY[i],observedT[i])
        ...
        # Fill in deltakx
        for j in range(nnew):
            self.deltakx[j][0] = self.kernelX.dkdy(self.observedT[n],self.newT[j])
            self.deltaky[j][0] = self.kernelY.dkdy(self.observedT[n],self.newT[j])
        # Fill in deltaKx
        for j in range(n+1):
            self.deltaKx[j][0] = self.kernelX.dkdy(self.observedT[n],self.observedT[j])
            self.deltaKy[j][0] = self.kernelY.dkdy(self.observedT[n],self.observedT[j])

    # Update single observation i
    def updateObserved(self,i,x,y,l):
        self.observedX[i][0] = x
        self.observedY[i][0] = y
        self.observedT[i][0] = l

    # Compute the likelihood
    def computeLikelihood(self,observedX,observedY,observedT,startG,finishG,stepsToCompare,goalsData):
        # TODO: remove the goalsData structure
        error = compute_prediction_error_of_points_along_the_path(stepsToCompare,observedX,observedY,observedT,startG,finishG,goalsData)
    ...
    def prediction_to_finish_point(self):
        ...
        for i in range(n):
            for j in range(nnew):
                self.kx[i][j] = self.kernelX(self.observedT[i],self.newT[j],False)
                self.ky[i][j] = self.kernelY(self.observedT[i],self.newT[j],False)
        # Fill in C
        for i in range(nnew):
            for j in range(nnew):
                self.Cx[i][j] = self.kernelX(self.newT[i],self.newT[j],False)
                self.Cy[i][j] = self.kernelY(self.newT[i],self.newT[j],False)
        ...
        return self.newX, self.newY, self.newT, self.varx, self.vary
    ...
    def prediction_to_perturbed_finish_point(self,deltax,deltay):
        ...
        return newx, newy, self.newT, self.varx, self.vary

    # Generate a sample from perturbations
    def sample_with_perturbation(self,deltaX,deltaY):
        # newx, newy, newt, varx, vary = self.prediction_to_finish_point() #we call this function to obtain newX, newY
