...
#Dado un valor en [0,1], regresa un punto (x,y) con x en [path.x_i-1, pathx_i]...
    ...
    while(i < n and j < m):
        ...
        if ti < tj:
            if ti > fj.t[j-1]:
                ...
                #print("Real: (",fi.x[i],",",fi.y[i],",",fi.t[i],")\nApproximation(",x,",",y,",",t,")")
            if i < n:
                i += 1
        elif tj < ti:
            if tj > fi.t[i-1]:
