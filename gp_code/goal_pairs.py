...
class goal_pairs:
	...
	# Constructor
	def __init__(self, goals_areas, trainingSet, coordinate_system, sigmaNoise=200.0, min_traj_number=5):
		...
		for i in range(self.goals_n):
			for j in range(self.goals_n):
				...
				if len(paths) > self.min_traj_number:
					...
					# Fit parameters in X
					thetaX  = fit_parameters(l,x,ker,theta,self.sigmaNoise,self.coordinate_system)
					...
					thetaY  = fit_parameters(l,y,ker,theta,self.sigmaNoise,self.coordinate_system)
