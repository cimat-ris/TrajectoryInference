...
def mlog_p(t,x,kernel,sigmaNoise):
	...
	# Get the log-determinant as the sum of the logs of the diagonal elements in C
	logDetK    = 0.0
	n          = len(x)
	for i in range(n):
...
# Evaluate minus sum of the log-likelihoods for all the trajectories
...
# Opimization of the parameters, given t and x
def fit_parameters(t,x,kernel,theta,sigmaNoise,coordinates):
	try:
		logging.debug("Call to fit")
		logging.debug("sigma noise: {}".format(sigmaNoise))
		if coordinates=='img':
			# Image coordinates
			parametersX = minimize(neg_sum_log_p,theta,(t,x,kernel,sigmaNoise),method='L-BFGS-B',options={'maxiter':100,'disp': True},bounds=((0.01,100.0),(0.01,1000.0)))
		else:
			# World coordinates
			parametersX = minimize(neg_sum_log_p,theta,(t,x,kernel,sigmaNoise),method='L-BFGS-B',options={'maxiter':100,'disp': True},bounds=((0.1,50.0),(1.0,50.0)))
		px          = parametersX.x
	except Exception as e:
		logging.error("[ERR] {:s} ".format(e))
		px = theta
	if parametersX.success != True:
		logging.error("[ERR] Optimization failed")		
