...
class path_regression:
	...
	def update_observations(self,observations,selection=True,consecutiveObservations=True):
		...
		logging.debug("Last observed point {}".format(lastObs))
		if lastObs[2] > 1:
			# Number of observations divided by l
			logging.debug("Number of observations {}".format(len(observations[:,2:3])))
			logging.debug("Last arc length {:2.2f}".format(lastObs[2]))
			self.nobs_per_length_unit = float(len(observations[:,2:3])/lastObs[2])
			logging.debug("Observations per length unit {:2.2f}".format(self.nobs_per_length_unit))
		...
		self.regression_x.update_observations(observations[:,0:1],observations[:,2:3],self.finalAreaCenter[0],finalL,(1.0-self.finalAreaAxis)*s*s*math.exp(-self.dist/s),self.predictedL,selection,consecutiveObservations)
		self.regression_y.update_observations(observations[:,1:2],observations[:,2:3],self.finalAreaCenter[1],finalL,    (self.finalAreaAxis)*s*s*math.exp(-self.dist/s),self.predictedL,selection,consecutiveObservations)

	# This function roughly estimates what the future arclengths of a trajectory will be
	def prediction_set_arclength(self, lastObs, goal):
		...
		euclideanDistToGoal = euclidean_distance([x,y], goal)
		logging.debug("Distance to goal {:2.2f}".format(euclideanDistToGoal))
		# Rough estimate of the remaining arc length
		# lengthToGoal    = euclideanDistToGoal*self.distUnit
		lengthToGoal    = euclideanDistToGoal
		logging.debug("DistUnit {:2.2f}".format(self.distUnit))
		logging.debug("Estimated length to goal {:2.2f}".format(lengthToGoal))
		size          = 1+int(lengthToGoal*self.nobs_per_length_unit)
		logging.debug("Points to predict {}".format(size))
		predset = np.zeros((size,1))
		if size > 0:
			step = lengthToGoal/float(size)
			for i in range(1,size+1):
				predset[i-1,0] = l + i*step
		return predset, l + lengthToGoal, lengthToGoal
	...
	def sample_path_with_perturbation(self,deltaX,deltaY,method=0):
		# Generate modified
		...
		deltaL = finalL_perturbed-self.regression_x.observedL[-1]
		logging.debug("Delta l {}".format(deltaL))
		if method==0:
			...
			predictedL,predictedX,__=self.regression_x.predict_to_perturbed_finish_point_interm(deltaL,deltaX)
			predictedL,predictedY,__=self.regression_y.predict_to_perturbed_finish_point_interm(deltaL,deltaY)
	...
	def sample_path_with_perturbed_finish_point(self,method=0):
		...
		return self.sample_path_with_perturbation(deltaX,deltaY,method)
