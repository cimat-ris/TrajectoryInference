...
class mGP_trajectory_prediction:
	...
	def update(self,observations,selection=True,consecutiveObservations=True):
		...
		for i in range(self.goalsData.goals_n):
			if self.gpTrajectoryRegressor[i] is not None:
				...
				self.gpTrajectoryRegressor[i].update_observations(observations,selection,consecutiveObservations)
	...
	def sample_path(self,method=0):
		...
		goal_x, goal_y, __ = uniform_sampling_1D(1, self.goalsData.goals_areas[end][1:], self.goalsData.goals_areas[end][0])
		logging.debug("Sampled goal {} {}".format(goal_x, goal_y))
